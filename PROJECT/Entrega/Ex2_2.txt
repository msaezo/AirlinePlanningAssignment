...
clear
close all
plots_on = 0; % 1 to plot, 0 to not plot. MAPPING TOOLBOX NEEDED. 
...
apdat.rwl = xlsread('input16_Ex2.xlsx',2,'C8:Z8'); % Runway length
apdat.slt = [1000 xlsread('input16_Ex2.xlsx',2,'D9:Z9')]; % Available slots

% Aircraft Data
acrft.num = xlsread('input16_Ex2.xlsx',2,'B12:F12'); % Available aircraft
acrft.vel = xlsread('input16_Ex2.xlsx',3,'B2:F2'); % Cruise speed
acrft.sts = xlsread('input16_Ex2.xlsx',3,'B3:F3'); % Seats available
acrft.tat = xlsread('input16_Ex2.xlsx',3,'B4:F4'); % Turn-around time
acrft.rng = xlsread('input16_Ex2.xlsx',3,'B5:F5'); % Range
acrft.rwl = xlsread('input16_Ex2.xlsx',3,'B6:F6'); % Runway length required
acrft.cl  = xlsread('input16_Ex2.xlsx',3,'B7:F7'); % Weekly lease costs
acrft.cx  = xlsread('input16_Ex2.xlsx',3,'B8:F8'); % Fixed operating costs
acrft.ct  = xlsread('input16_Ex2.xlsx',3,'B9:F9'); % Time costs parameter
acrft.cf  = xlsread('input16_Ex2.xlsx',3,'B10:F10'); % Fuel costs parameter

% Demand matrix
demnd = xlsread('input16_Ex2.xlsx',4,'C2:Z25');
...
% [newl_k cancel_k xij (direct pax) wij (connecting pax) zijk (flights offered)]

% function to translate i-j-k index to dv index
clear i j k % First 5 variables are for planes
varindex = @(i, j, k) 2*nacft + (i + (j-1)*nodes)*(k == -1) + ... % locates X_ij if k == -1
...
obj = [-lease - feenl; % Leasing costs (negative for new leases) - fees
     ...
     reshape(rvnum, nodes^2, 1); % Yield per passenger in route i,j
     reshape(rvnum, nodes^2, 1); % Yield per passenger in route i,j
    -reshape(costm, nacft*nodes^2,1)]; % Operating Costs
nvars = length(obj);
...
for i = 1:nacft
    namedv(i,:) = ['AC_' num2str(i,'%01d') '_newl'];
    namedv(i+nacft,:) = ['AC_' num2str(i,'%01d') '_cncl'];
end
for k = -1:nacft
    for j = 1:nodes
        for i = 1:nodes
    ...
    for j = 1:nodes
        ...
        for k = 1:nacft
            ...
            if (all_c) % If the constraints are not met, capacity is 0
                ...
                loadf = ldfeu + (ldfus-ldfeu)*((i > ndseu) || (j > ndseu)); %%CHANGED
...
for k = 1:nacft
    ...
    cplex.addRows(-Inf, C5, avgut*acrft.num(k), sprintf('Utilisation%d',k));
end
...
for i = ndseu+1:nodes % Flights from the us
    ...
    for j = 1:nodes % All destinations - even though only j=1 has z_ij != 0
        for k = 1:nacft % All aircraft types
            C7(varindex(i, j, k)) = acrft.sts(k);
            C8(varindex(j, i, k)) = acrft.sts(k);
        end
    end
end
cplex.addRows(0, C7, mxcap, 'MaxCap1');
cplex.addRows(0, C8, mxcap, 'MaxCap2');
...
[~, tags] = xlsread('input16_Ex2.xlsx',2,'C5:Z5');
tags = tags(1:nodes);
% Small violation of integer constraints ~1e-9
newl = sol(1:nacft)'; % New leases
canl = sol(nacft+1 : 2*nacft)'; % Cancelled Leases
AC   = acrft.num + newl - canl; % Aircraft number

sol  = sol(2*nacft+1:end);
...
tcost = sum(sum(sum(costm.*flts))) + sum(AC.*acrft.cl)  + sum(newl*feenl) + sum(canl*feecl);;
trvnu = sum(sum(rvnum.*(dpax+cpax)));
task  = sum(sum(cap.*distm)); % Total ASK
trpk  = sum(sum(fpax.*distm));% Total RPK
fprintf('\n\nKey Performance Indicators:\n');
fprintf('   ASK:  %10.0f ASK/week\n', task);
fprintf('   RPK:  %10.0f RPK/week\n', trpk);
...
xlswrite('input16_Ex2.xlsx',dpax,5,'B2:Y25');
xlswrite('input16_Ex2.xlsx',cpax,5,'B28:Y51');
xlswrite('input16_Ex2.xlsx',flts(:,:,1),5,'B54:Y77');
xlswrite('input16_Ex2.xlsx',flts(:,:,2),5,'B80:Y103');
xlswrite('input16_Ex2.xlsx',flts(:,:,3),5,'B106:Y129');
xlswrite('input16_Ex2.xlsx',flts(:,:,4),5,'B132:Y155');
xlswrite('input16_Ex2.xlsx',flts(:,:,5),5,'B158:Y181');
...
%     Gfl3 = digraph(flts(:,:,3),tags);% Sjet flights
    % Undirected Graphs
    Gdpax = graph(dpax,tags,'upper');% Direct Passenger flows
    Gcpax = graph(cpax,tags,'upper');% Connecting Passenger flows
    ...
    Gfl4 = graph(flts(:,:,4),tags);% Sjet flights
    ...
    % Direct, Connencting and total passenger flows - 1 plot
    ...
    figure('units','normalized','outerposition',[0 0 0.8 0.6])
    plot_graph_map({Gfl1,Gfl2,Gfl3,Gfl4}, lat, lon, maxval, ax, {'-','k','-','r'})
